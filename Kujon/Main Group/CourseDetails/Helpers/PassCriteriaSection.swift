...
class PassCriteriaSection: SectionHelperProtocol {
    ...
    func registerView(_ tableView: UITableView) {
        tableView.register(UINib(nibName: "GoFurtherViewCellTableViewCell", bundle: nil), forCellReuseIdentifier: criteriaCellId)
    }
    ...
    func giveMeCellAtPosition(_ tableView: UITableView, onPosition position: IndexPath) -> UITableViewCell! {
        let cell = tableView.dequeueReusableCell(withIdentifier: criteriaCellId, for: position) as! GoFurtherViewCellTableViewCell
