...
extension ProgrammeWithDifferentId{
    func getProgramme() -> Programme{
        return Programme(name: self.name, modeOfStudies: self.modeOfStudies, duration: self.duration, levelOfStudies: self.levelOfStudies, id: self.id , description: self.description)
    }
}